                    ...
                    <form method="POST" action="{{ route('appointments.store') }}">
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <select name="calendar_id" id="calendar_id" required
                                    ...
                                    @foreach ($calendars as $calendar)
                                        <option value="{{ $calendar->id }}" {{ old('calendar_id', $preselectedCalendar) == $calendar->id ? 'selected' : '' }}>
                                            {{ $calendar->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <input type="datetime-local" name="start_datetime" id="start_datetime" value="{{ old('start_datetime', $preselectedDate . 'T09:00') }}" required
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300">
                            ...
                            <div>
                                ...
                                <input type="datetime-local" name="end_datetime" id="end_datetime" value="{{ old('end_datetime', $preselectedDate . 'T10:00') }}" required
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300">
                                @error('end_datetime')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                                        ...
                                        <div class="flex items-start gap-3">
                                            <div class="flex items-center h-5">
                                                <input type="checkbox" name="reminders[{{ $minutes }}][enabled]" id="reminder_{{ $minutes }}" value="1"
                                                    {{ in_array($minutes, old('default_reminders', $defaultReminders ?? [15])) ? 'checked' : '' }}
                                                    class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                                            </div>
                                            <div class="flex-1">
